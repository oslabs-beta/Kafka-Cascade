...
class CascadeProducer extends EventEmitter {
    ...
    send(msg) {
        try {
            ...
            if (metadata.retries < this.retryTopics.length) {
                ...
                if (this.timeout[metadata.retries - 1] > 0)
                    return this.sendTimeout(id, producerMessage, metadata.retries - 1);
                else
                    return this.sendBatch(producerMessage, metadata.retries - 1);
            }
            else {
                this.emit('dlq', msg);
                this.dlqCB(msg);
    ...
    sendTimeout(id, msg, retries) {
        return new Promise((resolve, reject) => {
            ...
            if (process.env.test === 'test')
                ...
                setTimeout(scheduler, this.timeout[retries]);
        });
    }
    ;
    //Used by send
    ...
    setRetryTopics(topicsArr, options) {
        this.retryTopics = topicsArr;
        if (options && options.timeoutLimit)
            this.timeout = options.timeoutLimit;
        else
            this.timeout = (new Array(topicsArr.length)).fill(0);
        if (options && options.batchLimit)
            this.batchLimit = options.batchLimit;
        else
            this.batchLimit = (new Array(topicsArr.length)).fill(1);
